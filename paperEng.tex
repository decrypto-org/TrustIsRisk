...
\begin{document}
  ...
  \section{Definitions}
      ...
      \begin{definition}[Turns] \ \\
         ...
         \begin{enumerate}
            ...
            $DTr_{B \rightarrow A, j} = DTr_{B \rightarrow A, j-1} - y_B \wedge Cap_{A,j} = Cap_{A,j-1} + y_B$.
            ($Steal(y_B, B)$)
            \item Add value $y_B, -DTr_{A \rightarrow B, j-1} \leq y_B$ to $B \in N^{+}(A)$.
            $DTr_{A \rightarrow B, j} = DTr_{A \rightarrow B, j-1} + y_B \wedge Cap_{A,j} = Cap_{A,j-1} - y_B $. When
            $y_B < 0$, we say that $A$ reduces her trust to $B$ by $-y_B$, when $y_B > 0$, we say that $A$ increases her
            trust to $B$ by $y_B$. ($Add(y_B, B)$)
            \item Add value $y_B$ to $B \in \mathcal{M} \setminus N^{+}(A)$.
            Obviously $DTr_{A \rightarrow B, j-1} = 0, DTr_{A \rightarrow B, j} = y_B \wedge Cap_{A,j} = Cap_{A,j-1} - y_B$.
            We say that $A$ starts directly trusting player $B$ by $y_B$. ($AddNew(y_B, B)$)
         \end{enumerate}
         ...
         For example, $A$ cannot steal $y_{B,1} \leq DTr_{B \rightarrow A, j-1}$ and
         $y_{B,2} \leq DTr_{B \rightarrow A, j-1}$ from $B \in N^{-}(A)$ in the same turn, but can instead steal
         $y_{B,1} + y_{B,2}$ given that $y_{B,1} + y_{B,2} \leq DTr_{B \rightarrow A, j-1}$. Also $A$ is allowed
         to steal $y_B \leq DTr_{B \rightarrow A, j-1}$ from $B$ and start directly trusting the same player $B$ by
         $w_B$ given that $B \in N^{-}(A) \wedge B \notin N^{+}(A)$. \\
         The set of actions a player makes in turn $j$ is $Turn_j$. Examples:
         \begin{itemize}
            \item $Turn_{j_1} = \{Pass()\}$
            \item $Turn_{j_2} = \{Steal(y, B), AddNew(w, B)\}$ (given that $DTr_{B \rightarrow A, j_2 - 1} \leq y,
            DTr_{A \rightarrow B, j_2 - 1} = 0$, where $A = Player(j)$)
            \item $Turn_{j_3} = \{Steal(x, B), Add(y, C), AddNew(w, D)\}$ (given that $DTr_{B \rightarrow A, j_3 - 1} \leq x,
            DTr_{A \rightarrow C, j_3 - 1} \geq -y, DTr_{A \rightarrow D, j_3 - 1} = 0$, where $A = Player(j)$)
         \end{itemize}
      \end{definition}
      ...
      \begin{definition}[Honest strategy] \ \\
         ...
         More formally, if $Strategy(A) = Honest$, then $\forall j \in \mathbb{N}: Player(j) = A$ it is $Turn_j =
         \{Steal(y_1,B_1),...,Steal(y_m,B_m)\}$ where $\sum\limits_{i=1}^{m}Steal(y_i,B_i) = \max(0,
         \sum\limits_{i=1}^{|N^{+}(A)|}DTr_{A \rightarrow N^{+}(A)_i, j'} - \sum\limits_{i=1}^{|N^{+}(A)|}DTr_{A
         \rightarrow N^{+}(A)_i, j}), j' = \max\{k \in \mathbb{N} : k < j \wedge Player(k) = Player(j)\}$.
      \end{definition}
      \begin{definition}[Idle strategy] \ \\
         ...
         Idle$, then $\forall j \in \mathbb{N} : Player(j) = A$ it is $Turn_j = \{Pass()\}$.
      \end{definition}
      ...
      \begin{definition}[Indirect trust from $A$ to $B$, $Tr_{A \rightarrow B}$] \ \\
         ...
         and everyone else ($\mathcal{M} \setminus \{A,B\}$) follows the honest strategy. More formally,
         ...
         $Strategy(A) = Idle, Strategy(B) = Evil, \forall C \in \mathcal{M} \setminus \{A,B\} Strategy(C) = Honest$.
      \end{definition}
      \begin{definition}[Indirect trust from $A$ to $S \subset \mathcal{M}$, $Tr_{A \rightarrow S}$] \ \\
         ...
         the idle strategy and everyone else ($\mathcal{M} \setminus (S \cup \{A\}$) follows the honest strategy. More
         ...
         \forall C \in \mathcal{M} \setminus \{A,E\} Strategy(C) = Honest$.
      \end{definition}
  ...
  \section{Theorems-Algorithms}
    ...
    \label{convergence}
       ...
       Strategy(C) = Honest$ and $j_0 \in \mathbb{N} : Player(j_0) = B$. Given that all players eventually play, there exists
       a turn $j' > j_0 : \forall j \geq j', Turn_j = \{Pass()\}$.
    \end{theorem}
    \begin{proof}
       First of all, $\forall j > j_0 : Player(j) = B, Turn_j = \{Pass()\}$ because $B$ has already nullified his incoming
       ...
       $\forall j > j_0 : Player(j) = A, Turn_j = \{Pass()\}$ because of the idle strategy that $A$ follows. As far as the
       rest of the players are concerned, consider the following algorithm, where $RandomMember(Set)$ returns a randomly
       chosen member of the input set. \\
       \begin{algorithm}[H]
          \label{convergencealgo}
          ...
          $Ignorant \gets V(G) \setminus (Angry \cup Sad \cup\{A, B\})$ \\
          $\forall v \in Ignorant, Loss_v \gets 0$ \\
          $Happy \gets \emptyset$ \\
          $j \gets j_0$ \\
          \While{$Angry \neq \emptyset$}
             {$v \gets \RandomMember(Angry)$ \\
              $w \gets \RandomMember(\{w \in N^{-}(v) : DTr_{w \rightarrow v,j} > 0\})$ \\
              $exchange = \min(DTr_{w \rightarrow v,j}, Loss_v)$ \\
              $Turn_j \gets \{Steal(exchange, w)\}$ \\
              $DTr_{w \rightarrow v, j+1} = DTr_{w \rightarrow v, j} - exchange$ \\
              \If{$w \in Ignorant$}
                 {$Ignorant \gets Ignorant \setminus \{w\}$}
              \If{$w \in Happy$}
                 {$Happy \gets Happy \setminus \{w\}$}
              \If{$\sum\limits_{w' \in N^{-}(w)}DTr_{w' \rightarrow w, j} = 0$}
                 {$Sad \gets Sad \cup \{w\}$}
              \Else{$Angry \gets Angry \cup \{w\}$}
              \If{$v \in Angry \wedge \sum\limits_{v' \in N^{-}(v)}DTr_{v' \rightarrow v, j} = 0$}
...
\ \\ OLD
	   ...
	   \item We will show that $Tr_{A \rightarrow B} \leq MaxFlow_{A \rightarrow B}$.
          ...
          honest player will have been stolen value equal to $\sum\limits_{v \in N^{+}(w)}x_{wv}$. More concretely,
...
%        OLD PROOF END
    ...
    \begin{proof} \ \\
       ...
       Steal(y_m,B_m)\}$. This subseries must have an initial element, $Turn_{j_1}$. However, $Player(j_1)$ follows the honest
       strategy, thus somebody must have stolen from her as well, so $Player(j_1)$ cannot be the initial element. We have a
       contradiction, thus there cannot exist a series of stealing actions when everybody is honest.
    \end{proof}
    ...
    \end{theorem}       
    \begin{proof}
       If $T$ chooses the evil strategy and all players in $S$ play according to the honest strategy, they will have to steal
