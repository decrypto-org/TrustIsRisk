...
%       \item $\sum\limits_{v \in N^{+}(A)}x_{Av} = \sum\limits_{v \in N^{-}(B)}x_{vB}$
    \end{enumerate}
    \begin{lemma}[MaxFlows Are Games] \ \\
       \label{maxflowgame}
       ...
       algorithm \ref{transitivesteal} such that when it converges in turn $j_1 \in \mathbb{N}$, it holds that
       $maxFlow(A, B) \leq Loss_{A, j_1}$.
    \end{lemma}
    \begin{proof} \ \\
       ...
       $\forall v \in V(\mathcal{G}_0) \setminus \{A, B\}, \sum\limits_{w \in V(\mathcal{G}_0)}x_{wv} = \sum\limits_{w \in V(\mathcal{G}_0)}x_{vw}
       \leq maxFlow(A, B) \leq in_{B, 0}$. Player $B$ will follow a modified evil strategy where she steals value equal to
       her total incoming flow, not her total incoming trust. Let $j_2$ be the turn when $A$ is chosen to play. We will show
       using strong induction that there exists a set of
       valid actions for each player according to their respective strategy such that at the end of each turn $j$ the
       corresponding player $v = Player(j)$ will have stolen value $x_{wv}$ from each in neighbour $w$.
       \begin{itemize}
          \item Base case: In turn 1, $B$ steals value equal to $\sum\limits_{w \in V(\mathcal{G}_0)}x_{wB}$, following the
          modified evil strategy. $$Turn_1 = \{Steal(x_{v_1B}, v_1), \dots, Steal(x_{v_iB}, v_i), \dots,Steal(x_{v_mB},
          v_m)\},$$ where $m = |N^{-}(B)_1|$ and $\forall i \in [m], v_i = N^{-}(B)_{1, i}$.
          \item Induction hypothesis: Let $k \in [j_2 - 2]$. We suppose
          that $\forall j \in [k]$, there exists a valid set of actions, $Turn_j$, performed by $v = Player(j)$ such that $v$
          steals from each player $w$ value equal to $x_{wv}$. $$\forall j \in [k], Turn_j = \{Steal(x_{w_1v}, w_1), \dots,
          Steal(x_{w_iv}, w_i), \dots, Steal(x_{w_mv}, w_m)\},$$ where $m = |N^{-}(v)_j|$ and $\forall i \in [m], w_i =
          N^{-}(v)_{j, i}$.
          ...
          choose the following turn: $$Turn_j = \{Steal(x_{w_1v}, w_1), \dots, Steal(x_{w_iv}, w_i), \dots,
    ...
    \begin{lemma}[Games Are Flows] \ \\
       \label{gameflow}
       Let $\mathcal{G}_0$ be a game graph and $A, B \in V(\mathcal{G}_0)$ where $Strategy(A) = Idle \wedge Strategy(B) =
       Evil \wedge \forall v \in V(\mathcal{G}_0) \setminus \{A, B\}, Strategy(v) = Conservative$. Let also $\mathcal{H}$ be
       a history corresponding to an execution of algorithm \ref{transitivesteal} on the graph $\mathcal{G}_0$ and $j_1$ a
       turn when algorithm \ref{transitivesteal} has converged. Let $Sad, Happy, Angry$ be as defined in algorithm
       \ref{transitivesteal}.  There exists a valid flow
...
%          \item $\forall v \in V(\mathcal{G}_0) \setminus (Sad_{j_1} \cup \{A\}), c'_{Tv} = 0$
...
%------------MAY BE UNNECESSARY--------- \\
       ...
       \begin{itemize}
          \item $x'_{vw} \leq c'_{vw}$ (Requirement \ref{flow1} $\forall e \in E(\mathcal{G}_0)$)
...
%       There obviously [citation needed] exists a subflow from $A$ to $B$ equal to $Loss_{A, j_1}$. \\
    ...
    \begin{theorem}[Trust flow theorem] \ \\
       \label{trustflow}
       Let $\mathcal{G}_j$ be a game graph at a specific turn $j$ and $A, B \in V(\mathcal{G}_j)$. It holds that
       $Tr_{A \rightarrow B} = maxFlow(A, B)$.
    \end{theorem}
    \begin{proof} \
       From lemma \ref{maxflowgame} we see that there exists an execution of algorithm \ref{transitivesteal} such that
       $Loss_{A, j} = maxFlow(A, B)$. Since $Tr_{A \rightarrow B}$ is the maximum loss that $A$ can suffer after the
       convergence of algorithm \ref{transitivesteal}, we see taht $Tr_{A \rightarrow B} \geq maxFlow(A, B)$. \\
       There exists an execution of the Transitive Game such that $Tr_{A \rightarrow B} = Loss_{A, j}$. From lemma
       \ref{gameflow}, this execution corresponds to a flow. Thus $Tr_{A \rightarrow B} \leq maxFlow(A, B)$. \\
       We conclude that $Tr_{A \rightarrow B} = maxFlow(A, B)$.
    \end{proof}
