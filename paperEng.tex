...
%     \end{center}
     ...
     respectively. For a reference of common definitions, see Appendix. [Begin example]
...
while (True)
  ...
  \section{Trust Flow}
    ...
    \begin{proofsketch}
       ...
       that the total input is equal to the total output. For the complete proof, see Appendix (\ref{maxflowgameproof}).
    \end{proofsketch}
...
%        OLD PROOF END
     ...
     \label{riskinv}
        Let $\mathcal{G}$ game graph, $A, B \in \mathcal{V}, F = Tr_{A \rightarrow B}$ and $V \leq F$ the desired value to be
        transferred from $A$ to $B$. Let also $\mathcal{G}'$ such that
        \begin{align}
           \mathcal{V}' &= \mathcal{V} \\
           DTr'_{A \rightarrow B} &= DTr_{A \rightarrow B} + V \\
           \forall v \in \mathcal{V}' \setminus \{A\}, w \in \mathcal{V}'&, DTr'_{v \rightarrow w} = DTr_{v \rightarrow w}
           \enspace.
        \end{align}
        Then there exists an assignment for the outgoing trust of $A, DTr_{A \rightarrow v}$, such that
        \begin{equation}
           Tr'_{A \rightarrow B} = F \enspace.
        \end{equation}         
     \end{theorem}
     \begin{proof}
     \end{proof}
