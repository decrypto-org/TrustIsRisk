...
%              $w \gets$ \RandomMember{$\{w \in N^{-}(v) : DTr_{w \rightarrow v,j} > 0\}$} \\
       ...
       chosen to play that will not pass her turn. According to algorithm \ref{transitivestealalgo}, $A$ will either deplete
    ...
    \begin{proof} \ \\
       ...
       \end{enumerate}
       First we will show that each valid execution of algorithm \ref{transitivestealalgo} corresponds to a valid flow and
       afterwards we will show that the MaxFlow can be a result of a valid execution of \ref{transitivestealalgo}. Thus
       we will have proven that $Tr_{A \rightarrow B} = MaxFlow_{A \rightarrow B}$.
\ \\ OLD
       \begin{enumerate}
