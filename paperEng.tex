...
%       \SetKwInOut{Output}{Output}
    ...
    \begin{proof} \ \\
       ...
       \begin{itemize}
          ...
          DTr_{v \rightarrow B, 0}$ and $\sum\limits_{v \in N^{-}(B)_0}x_{vB} = maxFlow(A, B)$. The graph
          ...
          $maxFlow(A, B) \Rightarrow Loss_A = maxFlow(A, B)$.
          ...
          out_{A, 0} - out_{A, j}$. We create a new graph $FG'$ such that $V(FG') = V(FG) \cup T, E(FG') = E(FG) \cup
          \{(T, v) : v \in Sad_j\} \cup (T, A), \forall (v, w) \in E(FG), c'_{vw} = DTr_{v \rightarrow w, 0} - DTr_{v
          \rightarrow w, j}, \forall v \in Sad_j, c'_{Tv} = c'_{TA} = \infty$ ($T$ is an auxiliary source). We execute the
          $MaxFlow(T, B)$ algorithm on $FG'$ and we get a flow $X' : \forall v,w \in V(FG), x'_{vw} = c'_{vw}$ (with all the
          edges, except for the auxiliary ones, saturated). Thus $\sum\limits_{v \in N^{+}(A)_0}x_{Av} = Loss_{A, j}$.
          If we create a new graph $FG''$ with $V(FG'') = V(FG'), E(FG'') = E(FG'), \forall v \in Sad_j, c''_{Tv} = 0,
          c''_{TA} = \infty, \forall v,w \in V(FG), c''_{vw} = c_{vw}$ and execute the $MaxFlow(T, B) = X''$ algorithm on
          $FG''$, $\sum\limits_{v \in N^{+}(A)_0}x''_{Av} = \sum\limits_{v \in N^{+}(A)_0}x'_{Av}$ (the outgoing flow from
          $A$ will remain the same as in $FG'$) since no capacity accesible from $A$ has been modified (the only changed
          capacities are those that begin from $T$ and there is no incoming flow to $T$) and the outgoing flow from $A$ was
          already saturated. Thus the resulting flow is equal to $Loss_{A, j}$, or $\sum\limits_{v \in N^{+}(A)_0}x''_{Av} =
          \sum\limits_{v \in N^{+}(A)_0}c_{Av} = Loss_{A, j}$.
       \end{itemize}
...
%    \end{proof}
    ...
    \begin{proof}
       ...
       first and likewise for the second game. Let $T_1$ be an auxiliary player such that $\forall D \in \mathcal{B} \cup
       \mathcal{C}, DTr_{D \rightarrow T_1} = \infty$ and $T_2$ be another auxiliary player such that $\forall B \in
       \mathcal{B}, DTr_{B \rightarrow T_2} = \infty$. According to \ref{trustmany}, $Tr_{A \rightarrow \mathcal{B} \cup
       ...
       $\forall v \in \mathcal{V}_2 \setminus \mathcal{B}, \forall w \in \mathcal{V}_2, x_{vw,2} = x_{vw,1} \wedge \forall v
       \in \mathcal{B}, x_{vT_2,2} = \sum\limits_{w \in N^{+}(v)}x_{vw,1} \wedge \forall v,w \in \mathcal{B} \cup
       \mathcal{C}, x_{vw,2} = 0$ and the flow $X_2$ resulting from $maxFlow(A, T_2)$ is a valid flow for the first case if
       we set $\forall v \in \mathcal{V}_1 \setminus (\mathcal{B} \cup \mathcal{C}), \forall w \in \mathcal{V}_1, x_{vw,1} =
       x_{vw,2} \wedge \forall v \in \mathcal {B}, x_{vT_1,1} = \sum\limits_{w \in N^{+}(v)}x_{vw,2} \wedge \forall v \in
       \mathcal{C}, \forall w \in \mathcal{V}_1, x_{vw,1} = 0$. From these two observations, we deduce that the two
       $maxFlow$s are equal, because otherwise the lower $maxFlow$ of the two could be exchanged with the flow from the
       higher, thus it would not have been maximum in the first place. Thus $maxFlow(A, T_1) = maxFlow(A, T_2) \Rightarrow
