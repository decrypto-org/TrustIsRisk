...
\begin{document}
  ...
  \section{Definitions}
      \begin{definition}[Graph] \ \\
         ...
         $\mathcal{V}_j$ and a positive real number. More formally, $e \in \mathcal{E}_j \Rightarrow \exists A,B \in
         \mathcal{V}_j, w > 0 : e = (A,B,w)$.
      \end{definition}
      ...
      \begin{definition}[Direct Trust from $A$ to $B$ after turn $j$, $DTr_{A \rightarrow B, j}$] \ \\
         ...
         deposited by $A$. When $DTr_{A \rightarrow B, j} > 0, (A,B,DTr_{A \rightarrow B, j}) \in \mathcal{E}_j$. When
         $DTr_{A \rightarrow B, j} = 0, (A,B,0) \in \mathcal{E}_j$ or equivalently $\nexists w \in \mathbb{R} : (A,B,w) \in
         \mathcal{E}_j$.
      \end{definition}
...
%       \SetKwInOut{Output}{Output}
       ...
       \Input{old graph $\mathcal{G}_{j-1}$, old capital $Cap_{A, j-1}$, $ProvisionalTurn_j$}
%       \Output{new graph $\mathcal{G}_j$, new capital $Cap_{A, j}$, new history $\mathcal{H}_j$}
       \caption{Execute Turn}
       \executeTurn{$\mathcal{G}_{j-1}$, $Cap_{A, j-1}$, $ProvisionalTurn_j$} : \\ {
          $(Turn_j, NewCap) \gets$ \validateTurn{$\mathcal{G}_{j-1}$, $Cap_{A, j-1}$, $ProvisionalTurn_j$} \\
          \commitTurn{$mathcal{G}_{j-1}$, $NewCap$, $Turn_j$}} \\ \ \\
       \validateTurn{$\mathcal{G}_{j-1}$, $Cap_{A, j-1}$, $ProvisionalTurn_j$} : { \\
          $Y_{st} \gets 0$ \\
          $Y_{add} \gets 0$ \\
          \For{$action \in ProvisionalTurn_j$}{
          ...
          \If{$Y_{add} - Y_{st} > Cap_{A, j-1}$}{
             \Return{$\emptyset$, $Cap_{A, j-1}$}}
          \Else{\Return{$ProvisionalTurn_j$, $Cap_{A, j-1} + Y_{st} - Y_{add}$}}} \ \\
       \commitTurn{$\mathcal{G}_{j-1}$, $Cap_{A, j-1}$, $Turn_j$} : { \\
...
%       \SetKwInOut{Output}{Output}
       ...
       \While{True}{
          ...
          $ProvisionalTurn_j \gets \mathcal{O}_v()$ \\
          \executeTurn{$\mathcal{G}_{j-1}$,$Cap_{A, j-1}$,$ProvisionalTurn_j$}}
    \end{algorithm} \ \\
...
%          \Input{$A$ idle player, $B$ evil player, $j_0$ $B$'s turn}
...
%       \end{algorithm}
    \begin{algorithm}[H]
       ...
       \SetKwInOut{Output}{Output}
       \SetKwFunction{executeTurn}{executeTurn}
       \Input{$A$ idle player, $E$ evil player, $j_0$ $E$'s first turn}
       \Output{$\mathcal{H}$ history}
       \caption{Transitive Steal}
       $Turn_{j_0} \gets \mathcal{O}_E()$ \\ %\Comment{$\mathcal{O}_E = \mathcal{O}_{evil}$}\\
       \executeTurn{$\mathcal{G}_{j_0-1}$, $Cap_{E, j_0-1}$, $Turn_{j_0}$} \\
       $Angry \gets \emptyset$ \\
       $Happy \gets \emptyset$ \\
       $Sad \gets \emptyset$ \\
       $j \gets j_0$ \\
       \For{$v \in \mathcal{V}$}{
          \If{$v \in N^{-}(E)_{j_0-1}$}{
             $Loss_v \gets DTr_{v \rightarrow E, j_0-1}$ \\
             \If{$v \neq A$}{
                \If{$in_{v,j_0} > 0$}{
                   $Angry \gets Angry \cup \{v\}$}
                \Else{
                   $Sad \gets Sad \cup \{v\}$}}}
          \Else{\If{$v \neq A$}{
             $Happy \gets Happy \cup \{v\}$}}}
       \While{True}
          {$j \gets j + 1$ \\
           $v \overset{\$}{\gets} \mathcal{V}_j \setminus\{A,E\}$ \\
           $Turn_j \gets \mathcal{O}_v()$ \\ %\Comment{$\mathcal{O}_v = \mathcal{O}_{idle}$} \\
           \executeTurn{$\mathcal{G}_{j-1}$, $Cap_{v, j-1}$, $Turn_j$} \\
           \For{$w \in N^{-}(v)$}{
              \If{$\exists Steal(y, w) \in Turn_j$}{
                 $exchange \gets y : Steal(y, w) \in Turn_j$ \\
    ...
    \begin{proof} \ \\
       ...
       \begin{itemize}
          \item We will first that for any valid execution of algorithm \ref{transitivesteal} there exists at least one valid
          flow from $A$ to $B$, $X$, such that $Loss_A = \sum\limits_{v \in N^{+}(A)}x_{Av}$. Let $j$ be a turn where
          \ref{transitivesteal} has converged ($j$ exists, according to theorem \ref{convergence}). Then $Loss_{A, j} =
          out_{A, j_0} - out_{A, j}$. Let $\forall v \in N^{+}(A), x_{Av} = DTr_{A \rightarrow v, j_0} - DTr_{A \rightarrow
          v, j}$. For any conservative player $v \in N^{+}(A)$, let $\forall w \in N^{+}(v), x_{vw} \leq DTr_{v \rightarrow
          w, j_0} - DTr_{v \rightarrow w, j}, \sum\limits_{w \in N^{+}(A)}x_{vw} = x_{Av}$. This is possible because $v$ is
          conservative, thus the value she stole from $A$ must have been stolen previously from her. More generally, $\forall
          v \in \mathcal{V} \setminus \{A,B\}, \forall w \in N^{+}(v), x_{vw} \leq DTr_{v \rightarrow w, j_0} - DTr_{v
          \rightarrow w, j}, \sum\limits_{w \in N^{+}(v)}x_{vw} = \sum\limits_{w \in N^{-}(v)}x_{wv}$. Since the graph we
          build is a DAG, which corresponds to a partial order, there always exists a minimal element. Thus, by choosing to
          calculate the outgoing flows only of minimal elements, it is possible to create a valid flow network from $A$ to
          $B$ in exactly $|V(FG)| - 1$ iterations of the above steps.
          \item We will now show that there exists an execution of algorithm \ref{transitivesteal} such that $Loss_A =
          maxFlow_{A \rightarrow B}$. Let $X$ be the flows as returned by an execution of the $maxFlow_{A \rightarrow B}$
          algorithm. It is known that all flows are DAGs [citation needed] and that all DAGs have a maximal element [citation
          needed] based on the partial ordering $x_{vw} > 0 \Rightarrow v < w$. The maximal element is a node that does not
          have any outgoing flow. It is obvious that removing any node from a DAG cannot create a cycle, thus the graph that
          remains after removing a node from a DAG is also a DAG, thus is has a maximal node as well. If a player represented
          in turn $j > j_0$ by a maximal node $v$ is chosen to play for the first time by \ref{transitivesteal}, she is able
          to steal value equal at least to her outgoing flow on turn $j_0$, since she has already been stolen at least
          $\sum\limits_{w \in N^{+}(v)}x_{vw}$ (since she is maximal on turn $j$, she has no outgoing flow) and conform with
          the conservative strategy. Furthermore, she will pass her turn on all subequent turns she plays, since no
          additional value can be stolen from her because of her maximal status on turn $j$ (wrong). Thus if the algorithm
          chooses only maximal nodes, after exactly $|V(FG)| - 1$ turns (we do not count idle player $A$) every evil and
          conservative player will have stolen at least value equal to the flow passing through them in turn $j_0$ and player
          $A$ will have been stolen value exactly equal to $maxFlow_{A \rightarrow B} \Rightarrow Loss_A = maxFlow_{A
          \rightarrow B}$. \\
OLD
