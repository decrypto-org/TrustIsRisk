...
\begin{document}
  ...
  \section{Definitions}
      ...
      \begin{definition}[Indirect trust from $A$ to $B$, $Tr_{A \rightarrow B}$] \ \\
         ...
         $Strategy(A) = Idle, Strategy(E) = Evil, \forall C \in \mathcal{M} \setminus \{A,E\} Strategy(C) = Honest$.
      \end{definition}
      ...
      \begin{definition}[Restricted Flow] \ \\
         ...
         \begin{enumerate}
            \item Let $F_{A_i \rightarrow B}$ be the flow from $A$ to $N^{+}(A)_i$ as calculated by the $maxFlow(A,B)$
               ($x_i'$) when $u_i' = u_i,$ \\ $u_j' = 0 \:\forall j \in [|N^{+}(A)|] \wedge j \neq i$.
...
%        OLD PROOF END
    ...
    \label{trusttransfer}
       ...
       {\em \begin{lstlisting}
          ...
          /                                                   \
         /                                                     \
         $$Tr_{A \rightarrow E, j} = \max\limits_{j' : j' > j, configurations}{[\sum\limits_{i=1}^{|N^{+}(A)|}DTr_{A 
         \rightarrow N^{+}(A)_i,j}-\sum\limits_{i=1}^{|N^{+}(A)|}DTr_{A \rightarrow N^{+}(A)_i,j'}]}$$ where
         $Strategy(A) = Idle, Strategy(E) = Evil, \forall C \in \mathcal{M} \setminus \{A,E\} Strategy(C) = Honest$.
        / x_s2/u_s2                               x_2t/u_2t     \
       s-------------              ....          ------------t
...
%       \end{enumerate}
    ...
    player chooses the evil strategy, but without nullifying her outgoing direct trust.
...
%       Eve can steal from a player $A \in \mathcal{M} \setminus (\mathcal{B} \cup \mathcal{C})$ is $Tr_{A \rightarrow T}$.
    \end{theorem}
    \begin{proof}
       ...
       \{P_1, P_2\}$. The edges from $P_2$ to $P_1$ will carry a flow $X_P, X_{P'}$ and the edges inside of $P_2$ will carry
