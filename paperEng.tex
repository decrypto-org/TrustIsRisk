...
\maketitle
  ...
  \section{Definitions}
     ...
     \begin{definition}[Direct Trust from $A$ to $B$ after turn $j$, $DTr_{A \rightarrow B, j}$] \ \\
        ...
        direct trusts.
     \end{definition}
     \begin{definition}[(In/Out) Neighbourhood of $A$ on turn $j$] \
...
%      \end{definition}
     ...
     \begin{definition}[Conservative strategy] \ \\
        ...
        More formally, let $j' = prev(j), Damage_j = out_{A,j'} - out_{A,j-1}$ and $N^{-}(A)_j = \{v_1, \dots, v_k\}$. If
        $Strategy(A) = Conservative$, then $\forall j \in \mathbb{N}: Player(j) = A$ it is
        \begin{equation}
           Turn_j =
           \begin{cases}
              \emptyset, & Damage_j \leq 0 \\
              \bigcup\limits_{i=1}^{k}\{Steal(y_i,v_i)\}, & Damage_j > 0
           \end{cases}
        \end{equation}
...
%         except if it is otherwise denoted in some special cases. \\
     ...
     in_{A,j-1}$ she can choose to distribute the $Steal(s)()$ in any way she chooses, as long as
     \begin{equation}
        \forall i, y_i \leq DTr_{N^{-}(A)_{j,i} \rightarrow A, j-1} \wedge \sum\limits_{i=1}^{|N^{-}(A)_j|}y_i = Damage_j
        \enspace.
     \end{equation}
     ...
     We note that $N(A)_{j-1} = N(A)_j$. \\
     \Suppressnumber
     \begin{lstlisting}[label=conservativeoracle,style=numbers]
Conservative Oracle
Input : previous graph (*@$\mathcal{G}_{j-1}$@*)
Output : (*@$Turn_j$@*)
Ocons((*@$\mathcal{G}_{j-1}$@*)) : (*@\Reactivatenumber@*)
  NewOutTrust = (*@$\sum\limits_{v \in N^{+}(A)_{j-1}}DTr_{A \rightarrow v, j-1}$@*)
  NewInTrust = (*@$\sum\limits_{v \in N^{-}(A)_{j-1}}DTr_{v \rightarrow A, j-1}$@*)
  ...
  if (Damage > 0)
    if (Damage >= NewInTrust)
      ...
      (*@$(y_1, \dots, y_{|N^{-}(A)_{j-1}|})$@*) = SelectSteal((*@$DTr_{N^{-}(A)_{j-1,1} \rightarrow A, j-1} \dots
  ...
  \Return{$Turn_j$}}
     ...
     \begin{definition}[Evil strategy] \ \\
        ...
        \{Steal(y_1,N^{-}(A)_{j,1}),\dots,Steal(y_m,N^{-}(A)_{j,m}), Add(w_1,N^{+}(A)_{j,1}),\dots,Add(w_l,N^{+}(A)_{j,l})\}$ 
...
%       \SetKwInOut{Output}{Output}
    ...
    We continue with an important possible evolution of the game, the Transitive Game. In turn $0$, there is already a
    ...
    \begin{algorithm}[H]
       ...
       \SetKwInOut{Output}{Output}
       \SetKwFunction{executeTurn}{executeTurn}
       \Input{$\mathcal{G}_0$ game graph, $A \in \mathcal{V}_0$ idle player, $E \in \mathcal{V}_0$ evil player}
       \Output{$\mathcal{H}$ history}
       \caption{Transitive Game}
       $Angry \gets \emptyset$ \\
       $Happy \gets \emptyset$ \\
    ...
    $$Turn_j =
      \begin{cases}
         ...
         \bigcup\limits_{i=1}^{k}\{Steal(y_i,v_i)\}, & Damage_j > 0, N^{-}(A)_j = \{v_1,\dots,v_k\}
      \end{cases}$$
