...
%       \item $\sum\limits_{v \in N^{+}(A)}x_{Av} = \sum\limits_{v \in N^{-}(B)}x_{vB}$
    ...
    \begin{proof}
       ...
       \end{equation}
       The flow $X = X'' \setminus \{(T, A)\}$ is a possible output of $MaxFlow_{\mathcal{G}}\left(A, B\right)$ on graph
       $\mathcal{G}$. This holds for the following reasons:
       \begin{itemize}
          ...
          \end{equation}
          thus $X''$ is not $MaxFlow_{\mathcal{G}}\left(T, B\right)$, which is a contradiction.
