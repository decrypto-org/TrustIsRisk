...
%       \item $\sum\limits_{v \in N^{+}(A)}x_{Av} = \sum\limits_{v \in N^{-}(B)}x_{vB}$
    ...
    \begin{proof} \ \\
       ...
       that is a DAG [citation needed] and that we can apply the topological sort algorithm to any DAG and obtain a total
       ordering of its nodes with the following property: $\forall$ nodes $v, w$, it holds that $v < w \Rightarrow x_{wv} =
       0$ [citation needed]. Put differently, there is no flow from larger to smaller
