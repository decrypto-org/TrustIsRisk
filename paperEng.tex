...
%               $\cdot$ trust-as-risk $\cdot$ flow $\cdot$ reputation}
  ...
  \section{A detailed example}
     ...
     weight of which is the value which the head entrusts the tail with.
     \begin{center}
     \begin{tikzpicture}
     \begin{dot2tex}[dot,tikz,options=-s -tmath,scale=1.0]
        digraph G  {
           rankdir=LR;
           edge [arrowsize=0.6];
           node [shape=circle];
...
%     \end{center}
     ...
     transactions in the UTXO \cite{bitcoinguide} of which the private keys are controlled by the node. A rational player
     would like to maximize her capital in the long term. Let's suppose that player $A$ wants to buy a product that costs 10
     from player $B$, but $A$ has not given any direct trust to $B$. Does $A$'s friends trust $B$ enough so that $A$ can buy the product? In other
     words, should $B$ decide to steal all her incoming direct trust and stop trusting directly other users and the rest of
...
%        $\mathcal{V}_j$. More formally, $e \in \mathcal{E}_j \Rightarrow \exists A,B \in \mathcal{V}_j : e = \left(A,B\right)$.
     ...
     \begin{definition}[Capital of $A$, $Cap_A$] \ \\
        The capital of $A$, $Cap_A$ is defined as the total amount of value that exists in P2PKH in the UTXO and can be spent
        by $A$. We also define $Cap_{A,j}$ as the total amount of value that exists in P2PKH in the UTXO and can be spent by
        $A$ during turn $j$.
     \end{definition}
     \begin{definition}[Direct Trust from $A$ to $B$ After Turn $j$, $DTr_{A \rightarrow B, j}$] \ \\
        ...
        \begin{equation}
           DTr_{A \rightarrow B, j} =
              ...
              \end{cases}
        \end{equation}
        An algorithm that has access to the graph $\mathcal{G}_j$ has implicitly access to all direct trusts of this graph.
        The exception are the oracles, which in this case have access only to their incoming and outgoing direct trusts.
     \end{definition}
...
%, $N^{+}\left(A\right)_j, N^{-}\left(A\right)_j, N\left(A\right)_j$] \ 
     ...
     \end{definition}
     \begin{definition}[$Damage_{A, j}$] \ \\
        Let $j$ be a turn such that $Player\left(j\right) = A$.
        \begin{equation}
           Damage_{A, j} = out_{A, prev\left(j\right)} - out_{A, j}
        \end{equation}
     ...
     \begin{definition}[Idle Strategy] \ \\
        ...
        \begin{equation}
           \begin{rcases}
              Strategy\left(A\right) = Idle \\
              j \in \mathbb{N} : Player\left(j\right) = A
           \end{rcases}
           \Rightarrow Turn_j = \emptyset \enspace.
        \end{equation}
     \end{definition}
     \Suppressnumber
...
idleOracle((*@$\mathcal{G}_{j-1}$@*)) : (*@\Reactivatenumber@*)
  return((*@$\emptyset$@*))
     ...
     \begin{definition}[Evil Strategy] \ \\
        ...
        \begin{equation}
           \begin{rcases}
              Strategy\left(A\right) = Evil \\
              j \in \mathbb{N} : Player\left(j\right) = A
           \end{rcases}
           \Rightarrow Turn_j = Steals \cup Adds \enspace, \mbox{ where}
        \end{equation}
        \begin{equation}
        \begin{gathered}
           Steals = \bigcup\limits_{v \in N^{-}\left(A\right)_{j-1}}\{Steal(DTr_{v \rightarrow A, j-1}, v)\} \mbox{ and} \\
           Adds = \bigcup\limits_{w \in N^{+}\left(A\right)_{j-1}}\{Add\left(-DTr_{A \rightarrow w, j-1}, w\right)\}
           \enspace.
        \end{gathered}
        \end{equation}
...
evilOracle((*@$\mathcal{G}_{j-1}$@*)) : (*@\Reactivatenumber@*)
  ...
  return((*@$Turn_j$@*))
     ...
     \begin{definition}[Conservative Strategy] \ \\
        ...
        More formally, let $j' = prev\left(j\right), Damage_j = out_{A,j'} - out_{A,j-1}$ and $N^{-}\left(A\right)_j =
        \{v_1, \dots, v_k\}$. It holds that
        \begin{equation}
        \begin{gathered}
           \begin{rcases}
              Strategy\left(A\right) = Conservative \\
              j \in \mathbb{N}: Player\left(j\right) = A
           \end{rcases}
           \Rightarrow \\
           \Rightarrow Turn_j =
        ...
        \end{equation}
     \end{definition}
     The oracle remembers \texttt{PrevOutTrust} $= out_{A, j'}$ for  $j' = prev\left(j\right)$ and can observe all incoming
     and outgoing direct trusts of player $A$.
...
consOracle((*@$\mathcal{G}_{j-1}$@*)) : (*@\Reactivatenumber@*)
  NewOutTrust = (*@$out_{A, j-1}$@*)
  NewInTrust = (*@$in_{A, j-1}$@*)
  Damage = PrevOutTrust((*@$A$@*)) - NewOutTrust 
                                   #PrevOutTrust((*@$A$@*)) = (*@$out_{A, prev\left(j\right)}$@*)
  PrevOutTrust((*@$A$@*)) = NewOutTrust
  if (Damage > 0)
    if (Damage >= NewInTrust)
      ...
      (*@$y$@*) = SelectSteal((*@$G_j$@*), (*@$A$@*), Damage)    #(*@$y$@*) = ((*@$y_1$@*),...,(*@$y_{|N^{-}\left(A\right)_{j-1}|}$@*))
      (*@$Turn_j$@*) = (*@$\emptyset$@*)
  ...
  return((*@$Turn_j$@*))
     ...
     The rationale behind this strategy arises from a real-world common situation. Suppose there is a client, an intermediary
     and a producer. The client entrusts some value to the intermediary so that the latter can buy the desired product from
     the producer and deliver it to the client. The intermediary in turn entrusts an equal value to the producer, who needs
     the value upfront to be able to complete the production process. However the producer eventually does not give the
     product neither reimburses the value, due to bankruptcy or decision to exit the market with an unfair benefit. The
     intermediary can choose either to reimburse the client and suffer the loss, or refuse to return the money and lose the
     client's trust. The latter choice for the intermediary is exactly the conservative strategy. It is used throughout this
     work as a strategy for all the intermediary players because it models effectively the worst-case scenario that a client
     can face after an evil player decides to steal everything she can and the rest of the players do not engage in evil
     activity.
     \begin{definition}[Indirect Trust, $Tr_{A \rightarrow B, j}$] \ \\
        ...
        \end{equation}
     \end{definition}
     \begin{definition}[Indirect Trust to multiple players, $Tr_{A \rightarrow S, j}$] \ \\
...
while (True)
  ...
  (*@$Turn_j$@*) = (*@$v$@*)Oracle((*@$\mathcal{G}_{j-1}$@*))
  ...
  if ((*@$v$@*) != (*@$E$@*))
    ...
    has been stolen yet (see Appendix (\ref{conservativeworld}) for a formal proof of this simple fact). Moreover, let
    $j' = prev\left(j\right)$ and $\{v_1, \dots, v_k\} = N^{-}\left(A\right)_j$. Given that
    \begin{equation}
    ...
    \begin{equation}
       Turn_j =
         \begin{cases}
            ...
            \bigcup\limits_{i=1}^{k}\{Steal\left(y_i,v_i\right)\}, & Damage_{v, j-1} > 0
         \end{cases} \enspace.
    \end{equation}
    ...
    \begin{proofsketch}
       ...
       trust. For the complete proof, see Appendix (\ref{convergenceproof}).
    \end{proofsketch}
    ...
    \begin{proofsketch}
       ...
       The players will play in turns, moving from $E$ back to $A$. Each player will steal from his predecessors as much as
       was stolen from her. The flows and the conservative strategy share the property that the total input is equal to the
       total output. For the complete proof, see Appendix (\ref{maxflowgameproof}).
    \end{proofsketch}
    ...
    \begin{proofsketch}
       ...
       $A$ to $E$. For the complete proof, see Appendix (\ref{gameflowproof}).
    \end{proofsketch}
...
%        OLD PROOF END
    ...
    \begin{proof}
       ...
       $MaxFlow$ is concerned. The theorem follows thus from the Trust Flow Theorem. \qed
%       \begin{equation}
...
%       \end{equation}
    ...
    autonomy. Let Eve be a possible attacker.
  ...
  \section*{Appendix}
    ...
    \begin{proof}
       \begin{itemize}
          ...
          we ignore the argument (1).
       \end{itemize}
       Thus the theorem holds in every case. \qed
    \end{proof}
