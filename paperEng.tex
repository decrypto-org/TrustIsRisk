...
%     \end{equation}
     ...
     \begin{equation}
        Turn_j = \bigcup\limits_{w \in N^{-}\left(v\right)_{j-1}}\{Steal\left(y_w,w\right)\}, Damage_{v, j} > 0 \enspace,
     \end{equation}
     where
     \begin{equation*}
...
\end{tikzpicture}
  ...
  \section{Related Work}
     The topic of trust has been repeatedly attacked with several approaches: Cryptographic (pgp, pki, byzantine) where
     trust is rather binary, network trust with various assumptions (Mui, FIRE, Beta [centralized], CORE,
     DRMCSAMAS-Grunert, DTMAHP2PN - Repantis, Bazaar [centralized, maxflow], Beaver [decentralized ebay with fees], Web of
     Trust), frameworks that leave trust definition to be defined by use case (open reputation, PACE, GFMDRBTM - 4C),
     Synereo and PGP pathfinder have trust implications, Trust and Social Collateral gives a sociological justification for
     the design choice trust=risk and most related with our work are decentralized monetary trust
     systems (TrustDavis, Money as IOUs). The last two came close to our ideas, but were conceived before the advent of
  ...
  \section*{Appendix}
    ...
    \label{maxflowgameproof}
       ...
       \cite{flowdecomp}, which states that each flow can be seen as a finite set of paths from $A$ to $E$ and cycles, each
