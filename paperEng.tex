...
\begin{document}
  ...
  \section{Definitions}
      \begin{definition}[Players] \ \\
         The set $\mathcal{V} = V(G)$ is the set of all players in the network, otherwise understood as the set of all
      ...
      \begin{definition}[(In/Out) Neighbourhood of $A$, $N^{+}(A), N^{-}(A), N(A)$] \ 
         \begin{enumerate}
            \item Let $N^{+}(A)$ be the set of players $B$ that $A$ directly trusts with any positive value. More
               formally, $N^{+}(A) = \{B \in \mathcal{V} : DTr_{A \rightarrow B} > 0\}$. $N^{+}(A)$ is called out
               neighbourhood of $A$. Let also $S \subset \mathcal{V}$. $N^{+}(S) = \bigcup\limits_{A \in S}N^{+}(A)$.
            \item Let $N^{-}(A)$ be the set of players $B$ that directly trust $A$ with any positive value. More
               formally, $N^{-}(A) = \{B \in \mathcal{V} : DTr_{B \rightarrow A} > 0\}$. $N^{-}(A)$ is called in
               neighbourhood of $A$. Let also $S \subset \mathcal{V}$. $N^{-}(S) = \bigcup\limits_{A \in S}N^{-}(A)$.
            \item Let $N(A)$ be the set of players $B$ that either directly trust or are directly trusted by $A$ with
               ...
               neighbourhood of $A$. Let also $S \subset \mathcal{V}$. $N(S) = N^{+}(S) \cup N^{-}(S)$.
            \item Let $N(A)_i$ (respectively $N^{+}(A)_i, N^{-}(A)_i, N(S)_i, N^{+}(S)_i, N^{-}(S)_i, S \subset \mathcal{V}$)
               be the $i$-th element of set $N(A)$ (respectively of $N^{+}(A), N^{-}(A), N(S), N^{+}(S), N^{-}(S)$),
               according to an arbitrary but fixed enumeration of the set players.
         \end{enumerate}
      \end{definition}
...
%      \end{definition}
      ...
      \begin{definition}[Conservative strategy] \ \\
         ...
         equal to $\min{(x,\sum\limits_{B \in \mathcal{V}}{DTr_{B \rightarrow A}})}$ and she takes no other action.
         More formally, let $j' = \max\{k \in \mathbb{N} : k < j \wedge Player(k) = Player(j)\}, Damage = \sum\limits_{v \in
         N^{+}(A)}DTr_{A \rightarrow v, j'} - \sum\limits_{v \in N^{+}(A)}DTr_{A \rightarrow v,j-1}$. If $Strategy(A) =
         Conservative$, then $\forall j \in \mathbb{N}: Player(j) = A$ it is $$Turn_j =
         \begin{cases}
            \emptyset, & Damage \leq 0 \\
            \{Steal(y_1,N^{-}(A)_1),...,Steal(y_{|N^{-}(A)|},N^{-}(A)_{|N^{-}(A)|})\}, & Damage > 0
         \end{cases}$$
         In the second case, it is $\sum\limits_{i=1}^{|N^{-}(A)|}y_i = \min(\sum\limits_{v \in N^{-}(A)}DTr_{v \rightarrow
         A, j-1}, Damage)$. \\
...
%         Also, we define a conservative player's state as $Sad, Happy$ or $Angry$ as follows:
%         \begin{itemize}
%            \item If $Damage > 0 \wedge Damage > \sum\limits_{v \in N^{-}(A)}DTr_{v \rightarrow A} \Rightarrow Sad
%            \item 
%            \item 
%         \end{itemize}
      \end{definition}
      As we can see, the definition covers a multitude of options for the conservative player, since in case $0 < Damage <
      \sum\limits_{i=1}^{|N^{-}(A)|}DTr_{N^{-}(A)_i \rightarrow A, j-1}$ she can choose to distribute the $Steal(s)()$
      in any way she chooses, as long as $\forall i, y_i \leq DTr_{N^{-}(A)_i \rightarrow A, j-1} \wedge \sum\limits_{i=1}^
      {|N^{-}(A)|}y_i = Damage$.
      The oracle remembers $PrevOutTrust = \sum\limits_{v \in N^{+}(A)}DTr_{A \rightarrow v, j'}, j' = \max\{k \in \mathbb{N}
      : k < j \wedge Player(k) = Player(j)\}$ and can observe all incoming and outgoing direct trusts of player $A$,
      $\forall v \in N^{-}(A), DTr_{v \rightarrow A, j}, \forall v \in N^{+}(A), DTr_{A \rightarrow v, j}$. \\ 
      \begin{algorithm}[H]
         ...
         $NewOutTrust \gets \sum\limits_{v \in N^{+}(A)}DTr_{A \rightarrow v, j}$ \\
         $NewInTrust \gets \sum\limits_{v \in N^{-}(A)}DTr_{v \rightarrow A, j}$ \\
         $Damage \gets PrevOutTrust - NewOutTrust$ \\
         \If{$Damage > 0$}{
            \If{$Damage \geq NewInTrust$}{
               $Turn_j \gets \emptyset$ \\
               \For{$i \gets 1$ to $|N^{-}(A)|$}{
                  $Turn_j \gets Turn_j \cup \{Steal(DTr_{N^{-}(A)_i \rightarrow A, j-1}, N^{-}(A)_i)\}$}}
            \Else{
               $(y_1,...,y_{|N^{-}(A)|}) \gets$ \SelectSteal{$DTr_{N^{-}(A)_1 \rightarrow A, j-1}$,$...$,
                 $DTr_{N^{-}(A)_{|N^{-}(A)|} \rightarrow A, j-1}$,$Damage$} \\
               $Turn_j \gets \emptyset$ \\
               \For{$i \gets 1$ to $|N^{-}(A)|$}{
                  $Turn_j \gets Turn_j \cup \{Steal(y_i, N^{-}(A)_i)\}$}}}
         \Else{$Turn_j \gets \emptyset$}
         \Return{$Turn_j$}
      \end{algorithm}
      \texttt{SelectSteal()} returns $y_i, i \in [|N^{-}(A)|] : \forall i, y_i \leq DTr_{N^{-}(A)_i \rightarrow A},
      \sum\limits_{i=1}^{|N^{-}(A)|}y_i = Damage$. 
      ...
      \begin{algorithm}[H]
         ...
         \For{$i \gets 1$ to $|N^{-}(A)|$}{
            $Turn_j \gets Turn_j \cup \{Steal(DTr_{N^{-}(A)_i \rightarrow A, j-1}, N^{-}(A)_i)\}$}
         \For{$i \gets 1$ to $|N^{+}(A)|$}{
            $Turn_j \gets Turn_j \cup \{Add(-DTr_{A \rightarrow N^{+}(A)_i, j-1}, N^{+}(A)_i)\}$}
         \Return{$Turn_j$}
      \end{algorithm}
      ...
      \end{definition}
      \begin{definition}[Indirect trust from $A$ to $S \subset \mathcal{V}$, $Tr_{A \rightarrow S}$] \ \\
  ...
  \section{Theorems-Algorithms}
    \begin{algorithm}[H]
       ...
       \SetKwInOut{Output}{Output}
       \Input{old graph $G_{j-1}$, old capital $Cap_{A, j-1}$, $Turn_j$}
       \Output{new graph $G_j$, new history $\mathcal{H}_j$}
       ...
       \For{$action \in Turn_j$}{
          action match do \\ {
             case $Steal(y,w)$ do \\ {
                \If{$y > DTr_{w \rightarrow A} \vee y < 0$}{
                   \Return{$\bot$}}
                \Else{$Y_{st} \gets Y_{st} + y$}}
             case $Add(y,w)$ do \\ {
                \If{$y < -DTr_{w \rightarrow A}$}{
                   \Return{$\bot$}}
    ...
    \begin{proof}
       ...
       $$Turn_j =
         ...
         \end{cases}$$
       In the second case, it is $\sum\limits_{i=1}^{|N^{-}(v)|}y_i = \min(\sum\limits_{i=1}^{|N^{-}(v)|}DTr_{N^{-}(v)_i
       \rightarrow v, j-1}, Damage_{v,j-1})$. From the definition of $Damage_{v,j}$ and knowing that no strategy in this case
       ...
       conservative strategy. We will show that $$\min(\sum\limits_{i=1}^{|N^{-}(v)|}DTr_{N^{-}(v)_i \rightarrow v, j},
       Damage_{v,j}) = \min(\sum\limits_{i=1}^{|N^{-}(v)|}DTr_{N^{-}(v)_i \rightarrow v, j}, Loss_{v,j})$$ using induction.
       \begin{itemize}
          \item $j = j_0$ \\  $Damage_{v,j_0} = \sum\limits_{w \in N^{+}(v)}DTr_{v \rightarrow w, j_0 - 1} -
          \sum\limits_{w \in N^{+}(v)}DTr_{v \rightarrow w,j_0} = DTr_{v \rightarrow E, j_0 - 1}$ \\
          $Loss_{v,j_0} = DTr_{v \rightarrow E, j_0 - 1}$ (lines 29, 31). Thus $\forall v \in \mathcal{V} \setminus \{A,E\},
          Damage_{v,j_0} = Loss_{v,j_0} \Rightarrow$ \\ $\min(\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j_0},
          Damage_{v,j_0}) = \min(\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j_0}, Loss_{v,j_0})$.
          \item $j = j_1' : j_1' = \min\{k \in \mathbb{N} : k > j_1 \wedge Player(k) = Player(j_1)\}$
          \begin{itemize}
             \item $v_{j_1'-1} \in Happy$. Then
             \begin{enumerate}
                \item $v_{j_1'} \in Happy$ because $Turn_{j_1'} = \emptyset$,
                \item $Loss_{v, j_1'} = 0$ because otherwise $v_{j_1'} \notin Happy$,
                \item $Damage_{v, j_1'} = 0$, or else any reduction in direct trust to $v$ would increase equally
                $Loss_{v, j_1'}$ (line 9), which cannot be decreased but during an Angry player's turn (line 10).
                \item $\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v} \geq 0$
             \end{enumerate}
             Thus $\min(\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j_1'}, Damage_{v,j_1'}) = \min(\sum\limits_{w\in
             N^{-}(v)}DTr_{w \rightarrow v, j_1'}, Loss_{v,j_1'}) = 0$.
             \item $v_{j_1'-1} \in Sad$. Then
             \begin{enumerate}
                \item $v_{j_1'} \in Sad$ because $Turn_{j_1'} = \emptyset$, 
                \item $\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v,j_1'} = 0$ (lines 17-19, 28),
                \item $Damage_{v, j_1'} \geq 0 \wedge Loss_{v, j_1'} \geq 0$.
             \end{enumerate}
             Thus $\min(\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j_1'}, Damage_{v,j_1'}) = \min(\sum\limits_{w\in
             N^{-}(v)}DTr_{w \rightarrow v, j_1'}, Loss_{v,j_1'}) = 0$.
             \item $v_{j_1'-1} \in Angry \wedge v_{j_1'} \in Happy$. Then the same argument as in the first case holds, if
             we ignore the 1st argument.
             \item $v_{j_1'-1} \in Angry \wedge v_{j_1'} \in Sad$. Then the same argument as in the second case holds, if 
             we ignore the 1st argument.
          \end{itemize}
       \end{itemize}
       \begin{algorithm}[H]
          ...
          \While{$Angry \neq \emptyset$}
             {$v \gets$ \RandomMember{$Angry$} \\
              ...
              \If{$\sum\limits_{v' \in N^{-}(v)}DTr_{v' \rightarrow v, j} = 0 \wedge Loss_v > 0$}
          ...
          $\forall v \in (Sad \cup Angry), Loss_v \gets DTr_{v \rightarrow B, j_0 - 1}$ \\
       ...
       value stolen by $B$. Also, we can see in lines 14, 19 and 28, which are the only lines where the $Sad$ set is changed,
       that once a player enters the $Sad$ set, it is impossible to exit from this set. Also, we can see that players in
       $Sad \cup Happy$ are not chosen to play, which is equivalent to passing their turn. We will now show that eventually
       ...
       finite, thus this is possible only if $\exists j_1: \forall j \geq j_1, |Angry_j \cup Happy_j| = c > 0 \wedge
       |Angry_j| > 0$ (the total number of angry and happy players cannot increase because no player leaves the $Sad$ set
       and if it were to be decreased, it would eventually reach 0). Since $Angry_j \neq \emptyset$, a player $v$ will be
       chosen to play that will not pass her turn. According to algorithm \ref{transitivesteal}, $v$ will either deplete
       her incoming trust and enter the $Sad$ set, which is contradicting $|Angry_j \cup Happy_j| = c$, or will steal
       enough value to enter the $Happy$ set, that is $v$ will make $Loss_{v,j}=0$. Suppose that she has stolen $m$ players.
       They, in their turn, will steal total value at least equal to the value stolen by $v$ (since they cannot go sad).
