...
%               $\cdot$ trust-as-risk $\cdot$ flow $\cdot$ reputation}
  ...
  \section{Introduction}
     ...
     Two major examples of each are \href{http://www.ebay.com}{ebay} and \href{https://openbazaar.org/}{OpenBazaar}.
...
%     that the whole platform will be discarded as untrusted by the scammed party.
     ...
     trust-as-risk, or the proposition that $Alice$'s \textit{trust} to another user $Bob$ is defined to be the
     \textit{maximum sum of money} $Alice$ can lose when $Bob$ is free to choose any strategy she wants. To flesh out
     ...
     entrusted an amount of money to a third user, $Charlie$, then we trust indirectly $Charlie$ since if he wished to play
     unfairly, he could have already stolen the money entrusted by $Bob$. Thus we can engage in economic interaction with
     $Charlie$.
     \begin{center}
        \begin{tikzpicture}
     ...
     store on stars, ratings or other dubious and non-quantifiable trust measures. She can simply consult her wallet to
     decide whether the store is trustworthy and, if so, up to what value. Suppose that $Alice$ is viewing the item listings
     of vendor $Bob$. Instead of $Bob$'s stars, $Alice$ will see a positive value that is calculated by her wallet and
     represents the maximum monetary value that $Alice$ can safely use to complete a purchase from $Bob$. Obviously it will
     not be safe for $Alice$ to buy anything from $Bob$ or any vendor if she has entrusted no value to other players. We see
     that in TrustIsRisk the money is not invested at the time of the purchase and directly to the vendor, but at an earlier
     point in time and only to parties that are trustworthy for out-of-band reasons. The fact that this system can function
     in a completely distributed fashion will become clear in the following sections, along with the positive result that it
     is in principle Sybil resilient.
...
%     that will tautologically reduce their trust to her.
     ...
     its incoming and outgoing trust. Last but not least, the network can be viewed as a possible field for investment and
     speculation since it constitutes a completely new area for financial activity.
...
%        $\mathcal{V}_j$. More formally, $e \in \mathcal{E}_j \Rightarrow \exists A,B \in \mathcal{V}_j : e = \left(A,B\right)$.
     ...
     \begin{definition}[Turns] \ \\
        ...
        \begin{enumerate}
           \item Steal value $y_B, 0 \leq y_B \leq DTr_{B \rightarrow A, j-1}$ from $B \in N^{-}\left(A\right)_{j-1}$.
           \begin{equation}
