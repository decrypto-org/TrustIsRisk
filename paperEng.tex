...
%       \end{algorithm}
    \begin{algorithm}[H]
       ...
       \SetKwInOut{Output}{Output}
       \SetKwFunction{executeTurn}{executeTurn}
       \Input{$A$ idle player, $E$ evil player, $j_0$ $B$'s turn}
       \Output{$\mathcal{H}$ history}
       \caption{Transitive Steal}
    ...
    \begin{lemma}[$Loss$ equivalent to $Damage$] \ \\
       We will show that $Player(j) = v \Rightarrow in_{v, j} = 0 \vee Loss_{v, j} = Damage_{v, j}$.
    \end{lemma}
    \begin{proof} \ 
       \begin{itemize}
          ...
          \item $j \in \mathbb{N}: j > j_0$
          \begin{itemize}
             \item $v \in Happy_{j-1}$. Then
             \begin{enumerate}
                \item $v \in Happy_j$ because $Turn_{j_1'} = \emptyset$,
                \item $Loss_{v, j} = 0$ because otherwise $v \notin Happy_j$,
                \item $Damage_{v, j} = 0$, or else any reduction in direct trust to $v$ would increase equally
                $Loss_{v, j}$ (line 9), which cannot be decreased but during an Angry player's turn (line 10).
                \item $\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j} \geq 0$
             \end{enumerate}
             Thus $\min(\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v, j}, Damage_{v,j}) = \min(\sum\limits_{w \in
             N^{-}(v)}DTr_{w \rightarrow v, j}, Loss_{v,j}) = 0$.
             \item $v \in Sad_{j-1}$. Then
             \begin{enumerate}
                \item $v \in Sad_j$ because $Turn_{j} = \emptyset$, 
                \item $\sum\limits_{w \in N^{-}(v)}DTr_{w \rightarrow v,j} = 0$ (lines 17-19, 28),
    ...
    \begin{proof}
       ...
       rest of the players are concerned, consider the following algorithm, which is a variation of the TrustIsRisk Game. \\
