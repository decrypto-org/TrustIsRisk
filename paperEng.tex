...
%      \end{definition}
      ...
      \begin{definition}[Previous/Next turn of a player] \ \\
         ...
         that $A$ is chosen to play respectively. More formally, $$prev(j) = \max{\{k \in \mathbb{N} : k < j \wedge Player(k)
         = A\}}$$ $$next(j) = \min{\{k \in \mathbb{N} : k > j \wedge Player(k) = A\}}$$
      \end{definition}
...
%       \SetKwInOut{Output}{Output}
    ...
    Given that $Damage_{v,j} = out_{v,j'} - out_{v,j}$ where $j' = \max\{k \in \mathbb{N} : k < j \wedge Player(k) =
    Player(j)\}$, or $j' = 0$ if $j$ is the first turn in which $Player(j) = v$, the algorithm generates turns:
    $$Turn_j =
      \begin{cases}
    ...
    \end{lemma}
    \begin{proof} \ 
       $j \in \mathbb{N}: j > j_0$. $v = Player(j)$.
       \begin{itemize}
    ...
    \begin{proof} \ \\
       ...
       \begin{itemize}
          \item We will first show that for any valid execution of algorithm \ref{transitivesteal} there exists at least one
          ...
          remains after removing a node from a DAG is also a DAG, thus it has a maximal node as well. If a player represented
