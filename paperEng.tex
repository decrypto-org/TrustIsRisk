...
\end{tikzpicture}
  ...
  \section*{Appendix}
    ...
    \label{maxflowgameproof}
       ...
       seen as a finite set of $\left(A, B\right)$-paths and cycles, each having a certain flow. We execute $MaxFlow\left(A,
       B\right)$ and we apply the aforementioned Theorem. Obviously the cycles do not influence the
       $maxFlow\left(A, B\right)$, thus we can remove these flows. The resulting flow is a $MaxFlow\left(A, B\right)$ without
