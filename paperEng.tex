...
\begin{document}
  ...
  \section{Abstract}
     ...
     system in which direct trust is defined as lines-of-credit using bitcoin's 1-of-2 multisig. We introduce a new model for
     bitcoin wallets in which user coins are split among trusted friends.  Indirect trust is subsequently defined using a
  ...
  \section{Introduction}
     ...
     \href{http://www.ebay.com}{ebay}, which is centralized and as such it is vulnerable to ddos attacks [citation needed],
     has a single point of failure [citation needed], charges fees for the use of its services [citation needed] and
     maintains a database of all the transactions along with personal data [citation needed], but it can give money-back
     guarantees [citation needed] since it is run by a single company that has a financial advantage in keeping its clients
     satisfied. On the other hand, \href{https://openbazaar.org/}{OpenBazaar} is a fully decentralized platform built on
     bitcoin [citation needed], where individual stores or its \href{https://duosear.ch}{search engine} is vulnerable to ddos
     attacks [ciation needed], but not the platform as a whole. Additionally, it does not charge fees for its usage [citation
     needed] and there is no central agent recording all the transactions by name (except for the bitcoin blockchain)
     [citation needed] but it is possible for a store to scam a client and there exists no central authority able to verify
     the truth of the client's claim and reimburse her. Even though trust (or distrust) should be directed to each store
     individually, more often than not the whole platform is discarded as untrusted by the scammed client [citation needed].
...
%       \SetKwInOut{Output}{Output}
    ...
    \begin{proof} \ \\
       ...
       \begin{itemize}
          ...
          total order with an algorithm such as topoSort [citation needed]. The maximum element of the total order is a node
