...
%              $w \gets$ \RandomMember{$\{w \in N^{-}(v) : DTr_{w \rightarrow v,j} > 0\}$} \\
       ...
       in number and in value. More precisely, suppose that in $Turn_{j_1-1}, \sum\limits_{w,w' \in \mathcal{M}}DTr_{w
       ...
       \Rightarrow S_{j_n} < 0$. We have a contradiction because $\forall w,w' \in \mathcal{M}, DTr_{w \rightarrow w'}
