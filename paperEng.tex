...
%     that the whole platform will be discarded as untrusted by the scammed party.
     ...
     entrusted an amount of money to a third user, $Charlie$, then we trust indirectly $Charlie$ since if the latter wished
     ...
     not be safe for $Alice$ to buy anything from $Bob$ or any other vendor if she has entrusted no value to other players.
     ...
     function in a completely distributed fashion will become clear in the following sections, along with the positive result
     that it is in principle Sybil resilient.
...
%     that will tautologically reduce their trust to her.
     ...
     The initial members of TrustIsRisk can be groups of friends or researchers that are interested in this project. They
     can be responsible for the bootstrapping process. Furthermore, new and independent networks can arise for other business
     contexts. Apart from the common underlying blockchain, there needs to be no other type of connection between different
     TrustIsRisk networks.
     
  \section{A Formal Treatment}
...
%        $\mathcal{V}_j$. More formally, $e \in \mathcal{E}_j \Rightarrow \exists A,B \in \mathcal{V}_j : e = \left(A,B\right)$.
     ...
     the node directly and exclusively controls.
     \begin{definition}[Capital of $A$ at the End of Turn $j$, $Cap_{A, j}$] \ \\
        The capital of $A$ at the end of turn $j$, $Cap_{A, j}$, is defined as the total value that exists in P2PKH in the
        UTXO \cite{bitcoinguide} and can be spent by $A$ at the end of turn $j$. If the turn is obvious, we may omit the
        subscript $j$.
     \end{definition}
...
%     \end{tikzpicture}
%     \end{center}
     We use the notation $N^{+}(A)$ to refer to the nodes $A$ directly trusts and $N^{-}(A)$ for the nodes that directly
     trust $A$. For a formal definition, see Appendix (Definition \ref{neighbourhood}). We also use the notation $in_{A, j},
     out_{A, j}$ to refer to the total incoming and outgoing direct trust respectively. The formal definition can be found
     in the Appendix (Definition \ref{inouttrust}).
     Let's suppose that player $A$ wants to buy a product that costs 10
