...
%      \end{definition}
      \begin{definition}[Turns] \ \\
         ...
         turn to be feasible, it must hold that $Y_{st} - Y_{add} \leq Cap_{A,j-1}$. Moreover, player $A$ is not allowed to
      ...
      \begin{definition}[Conservative strategy] \ \\
         ...
         More formally, if $Strategy(A) = Conservative$, then $\forall j \in \mathbb{N}: Player(j) = A$ it is $Turn_j =
  ...
  \section{Theorems-Algorithms}
    ...
    \begin{proof}
       ...
       N^{-}(v)_i \rightarrow v,j-1}, Loss_{v,j-1})$. \\
       \begin{algorithm}[H]
