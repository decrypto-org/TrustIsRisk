...
%    \end{proof}
    ...
    \begin{proof}
       ...
       maxFlow(A, T), Tr_{A \rightarrow \mathcal{B} \cup \mathcal{C}} = maxFlow(A, T')$. From lemma \ref{mincutmany}, we know
       that the initial nodes of all edges in both $MinCut$s are outside $\mathcal{B} \cup \mathcal{C}$ and thus all edges in
       both $MinCut$s exist and have the same capacities in either case. Thus the flow $X$ resulting from $maxFlow(A, T)$ is
       a valid flow for the second case if we set $\forall v,w \in \mathcal{V} : v \in \mathcal{C} \vee w \in \mathcal{C},
       x_{vw} = 0$ and the flow $X'$ resulting from $maxFlow(A, T')$ is a valid flow for the first case if we exclude the
       flows of all the edges that have at least one end in $\mathcal{C}$. In both cases the $MinCut$ will be the same, thus
       $maxFlow(A, T) = maxFlow(A, T') \Rightarrow Tr_{A \rightarrow \mathcal{B}} = Tr_{A \rightarrow \mathcal{B} \cup
       \mathcal{C}}$.
