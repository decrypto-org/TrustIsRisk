...
\end{tikzpicture} \smallskip \ \\
  ...
  \section{The Trust Graph}
     ...
     \begin{definition}[Capital]
        The initial capital of $A$, $Cap_{A, 0}$, is defined as the total value that exists in P2PKH in the UTXO and can be
        spent by $A$ at the beginning of the game.
...
%        UTXO and can be spent by $A$ at the end of turn $j$.
     \end{definition}
     The capital of $A$ is subsequently modified only during her turns, according to her actions. A rational player would
     like to maximize her capital in the long term. We also define a player's assets:
...
\begin{scope}
  \definecolor{strokecol}{rgb}{0.0,0.0,0.0};
  \pgfsetstrokecolor{strokecol}
  \draw (133.5bp,11.5bp) node {\textbf{Fig.4:} Game Graph after $Turn_1$ (\ref{turnexample}) on the Graph of Figure 3};
\end{scope}
\begin{scope}
  \pgfsetstrokecolor{black}
  \definecolor{strokecol}{rgb}{1.0,1.0,1.0};
...
\end{scope}
  ...
  \draw (90.5bp,109.5bp) node {5\bitcoin};
  ...
  \draw (173.0bp,119.5bp) node {2\bitcoin};
%
\end{tikzpicture}
\end{center}
...
while (True)
  ...
  ((*@$G_j$@*), (*@$Cap_{v, j}$@*), (*@$H_j$@*)) = executeTurn((*@$\mathcal{G}_{j-1}$@*), (*@$v$@*), (*@$Cap_{v, j-1}$@*), Turn)
    \end{lstlisting}
...
%    \end{lstlisting}
  ...
  \section{Trust Transitivity}
     ...
     whenever we use the Transitive Game. For the algorithm, see the Appendix. An example follows:
\begin{center}
...
%    after turn 3. $B$ is happy until the end of turn 2 and sad after turn 3. The game converges in turn 4.
  \section{Trust Flow}
    ...
    \begin{proofsketch}
       ...
       trust.
%       For the complete proof, see the Appendix (proof \ref{convergenceproof}).
    \end{proofsketch}
...
%       For the complete proof, see the Appendix
    ...
    \begin{proofsketch}
       ...
       $A$ to $E$.
%       For the complete proof, see the Appendix (proof \ref{gameflowproof}).
    \end{proofsketch}
