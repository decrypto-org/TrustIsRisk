\section{Introduction}
  ...
  complete a purchase from $Charlie$. This value, known as indirect trust, is calculated with the Trust Flow
  theorem~\ref{trustflow}. Note that indirect trust towards a user is not global but subjective; each user views a
  ...
  already taking willingly. The existing voluntary risk is exactly that which $Alice$ was taking by sharing her coins with her
  trusted friends. We prove this result in the Risk Invariance theorem~\ref{riskinv}. Obviously it will not be safe for
  $Alice$ to buy anything from $Charlie$ or any other vendor
